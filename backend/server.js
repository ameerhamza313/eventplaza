...
app.post('/api/events', (req, res) => {
  const { eventName, eventDate, eventTime, eventLocation, eventDescription, eventCategory, registrationDeadline, maxAttendees, ticketPrice } = req.body;

  if (!eventName || !eventDate || !eventLocation || !eventDescription || !eventCategory) {
    return res.status(400).json({ error: 'Event name, date, location, description, and category are required.' });
  }
...
app.put('/api/events/:eventId', (req, res) => {
  ...
  try {
    ...
    const eventIndex = events.findIndex((event) => event.eventId === parseInt(eventId));
    if (eventIndex === -1) {
...
app.post('/send-email', async (req, res) => {
  try {
    ...
    const qrCodeData = await generateQRCodeData(bookingId, fullName, email, ticketQuantity);
...
const generateQRCodeData = async (bookingId, fullName, email, ticketQuantity) => {
